...
export async function getTasks(token: string, childId: string) {
  const url = `${API}/task/child/${childId}`;
...
export async function undoTask(token: string, taskId: string) {
  const res = await fetch(`${API}/task/${taskId}/undo`, {
    method: "PATCH",
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error((await res.json()).error || "Failed to undo task");
  return res.json();
}
